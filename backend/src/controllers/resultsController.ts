...
async function getSeasonConfig(
  round: string,
): Promise<{ eliteIds: string[]; bottomTeamIds: string[] }> {
  let eliteIds: string[] = [];
...
export const getResultsByRound = async (req: Request, res: Response) => {
  try {
    ...
    if (!Round) {
      return res
        .status(400)
        .json({ message: "Le numéro du round est requis." });
    }
    ...
    if (dbResults.rows.length === 0) {
      return res
        .status(404)
        .json({
          message: "Les résultats de ce round ne sont pas encore disponibles.",
        });
    }
    ...
    const allDriver = myData.map((drivers) => {
      ...
      const malusScore = ScoringEngine.calculateUnderdogBonus(
        drivers,
        bottomTeamIds,
      );
...
export const getSelectedRanking = async (req: any, res: any) => {
  try {
    ...
    if (
      !teamResult.rows[0] ||
      !teamResult.rows[0].driver_ids ||
      teamResult.rows[0].driver_ids.length === 0
    ) {
    ...
    const allConfigs = await db.query(
      "SELECT round, key, value FROM public.season_config",
    );
    const configMap: Record<
      string,
      { eliteIds: string[]; bottomTeamIds: string[] }
    > = {};
    ...
    Object.keys(raceResultsByRound).forEach((round) => {
      ...
      selectedDrivers.forEach((driverId) => {
        ...
        const driverData = raceResults.find(
          (r) => r.driverId.toLowerCase() === driverKey,
        );

        if (driverData && cumulativeRanking[driverKey]) {
          const baseScore = ScoringEngine.calculateUserPoints(
            driverData,
            eliteIds,
          );
          const bonusDepassement =
            ScoringEngine.calculateOvertakeBonus(driverData);
          const bonusEquipe = ScoringEngine.calculateTeamMateBonus(
            driverData,
            raceResults,
          );
          const malusScore = ScoringEngine.calculateUnderdogBonus(
          ...
          const roundTotal =
            baseScore + bonusDepassement + bonusEquipe + malusScore;

          cumulativeRanking[driverKey].totalPoints += roundTotal;
          cumulativeRanking[driverKey].constructorId = driverData.constructorId;
          cumulativeRanking[driverKey].positionsHistory.push(
            Number(driverData.position),
          );
        }
      });
    });
    ...
    return res
      .status(500)
      .json({ message: "Erreur lors du calcul du classement personnalisé." });
  }
};

// recuperer la fiche des scores detaillée
export const getUserRoundDetails = async (req: any, res: Response) => {
  try {
    ...
    if (dbResults.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Données indisponibles pour ce Grand Prix." });
    }

    const raceResults: RaceResult[] = dbResults.rows;
    const { eliteIds, bottomTeamIds } = await getSeasonConfig(Round);

    const driversDetails = selectedDrivers
      .map((driverId) => {
        const driverData = raceResults.find(
        ...
        try {
          const basePoints = ScoringEngine.calculateUserPoints(
            driverData,
            eliteIds,
          );
          const bonusOvertake =
            ScoringEngine.calculateOvertakeBonus(driverData);
          const bonusDuel = ScoringEngine.calculateTeamMateBonus(
            driverData,
            raceResults,
          );
          const bonusUnderdog = ScoringEngine.calculateUnderdogBonus(
            driverData,
            bottomTeamIds,
          );
    ...
    const computedTotal = Math.max(
      0,
      driversDetails.reduce((sum, d) => sum + d.total, 0),
    );
    ...
    return res
      .status(500)
      .json({ message: "Erreur lors du calcul unifié des scores du round." });
  }
};

// calculer automatiquement le classement d'un round demandé
  ...
  try {
    ...
    const leaderboard = userTeamsQuery.rows.map((row) => {
      ...
      drivers.forEach((driverId) => {
        ...
        if (driverData) {
          totalPoints +=
            ScoringEngine.calculateUserPoints(driverData, eliteIds) +
            ScoringEngine.calculateOvertakeBonus(driverData) +
    ...
    return res
      .status(500)
      .json({ message: "Erreur lors du calcul automatique du classement." });
  }
};
...
export const getSeasonLeaderboard = async (req: Request, res: Response) => {
  try {
    ...
    userTeamsQuery.rows.forEach((row) => {
      ...
      const drivers: string[] = row.driverIds || [];
      const raceResults = raceResultsByRound[round] || [];
      const currentConfig = configMap[round] || {
        eliteIds: [],
        bottomTeamIds: [],
      };
      ...
      userScoresMap[row.username] =
        (userScoresMap[row.username] || 0) + Math.max(0, roundPoints);
    });
    ...
    return res
      .status(500)
      .json({ message: "Erreur lors du calcul du classement général." });
  }
};
